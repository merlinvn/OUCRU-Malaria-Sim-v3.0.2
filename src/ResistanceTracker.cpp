...
void make_resistance_profile(std::vector<int> &vResistanceID, std::vector<int> &drugs_used, DrugTypePtrMap &drug_db,
                             const int &size) {
    vResistanceID.clear();
...
void make_arterminsinin_resistance_profile(std::vector<int> &vResistanceID, std::vector<int> &drugs_used,
                                           DrugTypePtrMap &drug_db) {
    ...
    for (GenotypePtrMap::iterator it = Model::CONFIG->parasite_db()->genotype_db().begin();
