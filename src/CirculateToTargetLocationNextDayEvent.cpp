...
CirculateToTargetLocationNextDayEvent::CirculateToTargetLocationNextDayEvent(
        const CirculateToTargetLocationNextDayEvent &orig) {
}
    ...
    if (target_location_ != person->residence_location()) {
        ...
        if (!person->has_return_to_residence_event()) {
            ...
            while (length_of_trip < 1) {
                length_of_trip = Model::RANDOM->random_gamma(Model::CONFIG->spatial_information().length_of_stay_theta,
                                                             Model::CONFIG->spatial_information().length_of_stay_k);
            }
