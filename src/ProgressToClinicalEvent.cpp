...
void ProgressToClinicalEvent::execute() {
    ...
    double density = Model::RANDOM->random_normal_truncated(
            Model::CONFIG->log_parasite_density_level().log_parasite_density_clinical, 0.1);
...
//        std::cout << "Hello" << std::endl;
        ...
        if (clinical_caused_parasite_->resist_to(therapy)) {
            ...
            person->schedule_test_treatment_failure_event(clinical_caused_parasite_, therapy->testing_day(), false,
                                                          therapy->id());
        }
