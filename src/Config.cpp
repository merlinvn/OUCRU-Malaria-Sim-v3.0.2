...
Config::Config(Model *model) : model_(model), strategy_(NULL), drug_db_(NULL), parasite_db_(NULL),
                               initial_parasite_info_(), importation_parasite_info_(),
                               importation_parasite_periodically_info_(), tme_strategy_(NULL) {
    total_time_ = -1;
...
Strategy *Config::read_strategy(const YAML::Node &config, const YAML::Node &n, const std::string &strategy_name) {
    ...
    if (strategy_name == "CyclingStrategy") {
        ...
        ((AdaptiveCyclingStrategy *) s)->set_delay_until_actual_trigger(
                n[strategy_name]["delay_until_actual_trigger"].as<int>());
...
void Config::calculate_relative_biting_density() {
    ...
    for (int i = 0; i < relative_bitting_information_.v_biting_level_density.size(); i++) {
        relative_bitting_information_.v_biting_level_density[i] =
                relative_bitting_information_.v_biting_level_density[i] +
                (1 - sum) / relative_bitting_information_.v_biting_level_density.size();
...
void Config::read_spatial_info(const YAML::Node &config) {
    ...
    for (int i = 0; i < spatial_information_.v_moving_level_density.size(); i++) {
        spatial_information_.v_moving_level_density[i] = spatial_information_.v_moving_level_density[i] +
                                                         (1 - sum) / spatial_information_.v_moving_level_density.size();
...
void Config::read_spatial_external_population_info(const YAML::Node &config) {
    ...
    for (int i = 0; i < spatial_external_population_information_.v_moving_level_density.size(); i++) {
        spatial_external_population_information_.v_moving_level_density[i] =
                spatial_external_population_information_.v_moving_level_density[i] +
                (1 - sum) / spatial_information_.v_moving_level_density.size();
    ...
    assert(spatial_external_population_information_.number_of_moving_levels ==
           spatial_external_population_information_.v_moving_level_density.size());
    assert(spatial_external_population_information_.number_of_moving_levels ==
           spatial_external_population_information_.v_moving_level_value.size());
    assert(fabs(t - 1) < 0.0001);

    for (int i = 0; i < number_of_locations_; i++) {
        spatial_external_population_information_.circulation_percent.push_back(
    ...
    if (Model::RANDOM != NULL) {
        Model::RANDOM->external_population_moving_level_generator().set_level_density(
                &spatial_external_population_information_.v_moving_level_density);
    }
    ...
    for (int i = 0; i < n.size(); i++) {
        ...
        if (location < number_of_locations_) {
            for (int j = 0; j < n[i]["parasite_info"].size(); j++) {
                ...
                int num = n[i]["parasite_info"][j]["number_of_cases"].as<int>();
                importation_parasite_periodically_info_.push_back(
                        ImportationParasitePeriodicallyInfo(location, parasite_type_id, dur, num));
            }
        }
    }
