...
namespace Spatial {
    ...
    BarabasiSM::~BarabasiSM() {

    }

    DoubleVector BarabasiSM::get_v_relative_outmovement_to_destination(const int &from_location,
                                                                       const DoubleVector &relative_distance_vector,
                                                                       const IntVector &v_number_of_residents_by_location) const {
        DoubleVector v_relative_number_of_circulation_by_location(Model::CONFIG->number_of_locations(), 0);
