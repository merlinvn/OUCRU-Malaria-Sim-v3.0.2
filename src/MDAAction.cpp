...
void MDAAction::perform_weekly_action(const int &location) {
    ...
    double percent_MDA_treated_per_day = (Model::CONFIG->tme_info().MDA_coverage[location] /
                                          Model::CONFIG->tme_info().MDA_duration[location]);
    for (int state = 0; state < Person::NUMBER_OF_STATE - 1; state++) {
        for (int ac = 0; ac < Model::CONFIG->number_of_age_classes(); ac++) {
            for (int i = 0; i < pi_lsa->vPerson()[location][state][ac].size(); i++) {
                ...
                ReceiveMDADrugEvent::schedule_event(Model::SCHEDULER, p, Model::CONFIG->tme_strategy()->get_therapy(),
                                                    Model::SCHEDULER->current_time() + day);
