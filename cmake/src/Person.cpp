...
Person::Person() :
location_(-1), residence_location_(-1), age_(-1), age_class_(-1), host_state_(SUSCEPTIBLE), birthday_(-1), bitting_level_(-1), base_bitting_level_value_(0), moving_level_(-1), external_population_moving_level_(-1), liver_parasite_type_(NULL), latest_update_time_(-1), number_of_times_bitten_(0), last_therapy_id_(0), is_moving_to_external_population_(false) {
    population_ = NULL;
    immune_system_ = NULL;
    all_clonal_parasite_populations_ = NULL;
    drugs_in_blood_ = NULL;

    today_infections_ = NULL;
    today_target_locations_ = NULL;
...
void Person::set_location(const int &value) {
    if (location_ != value) {
        ...
        if (Model::DATA_COLLECTOR != NULL) {
            if (location_ != -1) {
                Model::DATA_COLLECTOR->update_person_days_by_years(location_, -(365 - Model::SCHEDULER->current_day_in_year()));
            }
...
ClonalParasitePopulation* Person::add_new_parasite_to_blood(IntGenotype* parasite_type) {
    ...
    bloodParasite->set_last_update_log10_parasite_density(Model::CONFIG->log_parasite_density_level().log_parasite_density_from_liver);

    return bloodParasite;
}

void Person::notify_change_in_force_of_infection(const double& sign, const int &parasite_type_id, const double &weight, const double &log_total_relative_parasite_density) {
    if (weight == 0.0) {
    ...
    double relative_force_of_infection = sign * get_biting_level_value() * relative_infectivity(log_total_relative_parasite_density) * weight;
...
double Person::relative_infectivity(const double &log10_parasite_density) {
    double d_n = log10_parasite_density * Model::CONFIG->relative_infectivity().sigma + Model::CONFIG->relative_infectivity().ro_star;
...
void Person::cancel_all_other_progress_to_clinical_events_except(Event* event) {
    ...
    BOOST_FOREACH(Event* e, *events()) {
        if (e != event && dynamic_cast<ProgressToClinicalEvent*> (e) != NULL) {
    ...
    if (scTherapy != NULL) {
        ...
        for (int i = 0; i < macTherapy->therapy_ids().size(); i++) {
            ...
            if (start_day == 1) {
                ...
                assert(start_day > 1);
                ReceiveTherapyEvent::schedule_event(Model::SCHEDULER, this, Model::CONFIG->therapy_db()[therapy_id], Model::SCHEDULER->current_time() + start_day - 1, clinical_caused_parasite_);
            }
        }
    }
...
void Person::schedule_update_by_drug_event(ClonalParasitePopulation* clinical_caused_parasite) {
    ...
    UpdateWhenDrugIsPresentEvent::schedule_event(Model::SCHEDULER, this, clinical_caused_parasite, Model::SCHEDULER->current_time() + 1);
}
...
void Person::schedule_end_clinical_event(ClonalParasitePopulation* blood_parasite) {
    ...
    EndClinicalEvent::schedule_event(Model::SCHEDULER, this, blood_parasite, Model::SCHEDULER->current_time() + dClinical);
}
...
void Person::schedule_end_clinical_by_no_treatment_event(ClonalParasitePopulation* blood_parasite) {
    ...
    EndClinicalByNoTreatmentEvent::schedule_event(Model::SCHEDULER, this, blood_parasite, Model::SCHEDULER->current_time() + dClinical);

}

void Person::change_state_when_no_parasite_in_blood() {
    if (all_clonal_parasite_populations_->size() == 0) {
        if (liver_parasite_type_ == NULL) {
            //        std::cout << "S" << std::endl;
...
void Person::determine_relapse_or_not(ClonalParasitePopulation* clinical_caused_parasite) {
    if (all_clonal_parasite_populations_->contain(clinical_caused_parasite)) {
        ...
        if (P <= Model::CONFIG->p_relapse()) {
            ...
            clinical_caused_parasite->set_last_update_log10_parasite_density(Model::CONFIG->log_parasite_density_level().log_parasite_density_asymptomatic);
            schedule_relapse_event(clinical_caused_parasite, Model::CONFIG->relapse_duration());

        } else {
            //progress to clearance
            if (clinical_caused_parasite->last_update_log10_parasite_density() > Model::CONFIG->log_parasite_density_level().log_parasite_density_asymptomatic) {
                clinical_caused_parasite->set_last_update_log10_parasite_density(Model::CONFIG->log_parasite_density_level().log_parasite_density_asymptomatic);
            }
...
void Person::schedule_relapse_event(ClonalParasitePopulation* clinical_caused_parasite, const int& time_until_relapse) {
    ...
    ProgressToClinicalEvent::schedule_event(Model::SCHEDULER, this, clinical_caused_parasite, Model::SCHEDULER->current_time() + duration);
}
...
void Person::update_bitting_level() {
    if (Model::CONFIG->using_age_dependent_bitting_level()) {
        ...
        int diff_in_level = floor(new_bitting_level_value - get_biting_level_value()) / ((Model::CONFIG->relative_bitting_information().max_relative_biting_value - 1) / (double) (Model::CONFIG->relative_bitting_information().number_of_biting_levels - 1));
        if (diff_in_level != 0) {
...
void Person::update_current_state() {
    ...
    //clear drugs <=0.1
    drugs_in_blood_->clear_cut_off_drugs_by_event(NULL);
...
void Person::schedule_mature_gametocyte_event(ClonalParasitePopulation* blood_parasite) {
    int day_mature_gametocyte = (age_ <= 5) ? Model::CONFIG->days_mature_gametocyte_under_five() : Model::CONFIG->days_mature_gametocyte_over_five();
    MatureGametocyteEvent::schedule_event(Model::SCHEDULER, this, blood_parasite, Model::SCHEDULER->current_time() + day_mature_gametocyte);
}
...
void Person::schedule_move_to_target_location_next_day_event(const int& location) {
    CirculateToTargetLocationNextDayEvent::schedule_event(Model::SCHEDULER, this, location, Model::SCHEDULER->current_time() + 1);
}

bool Person::has_return_to_residence_event() {

    BOOST_FOREACH(Event* e, *events()) {
        if (dynamic_cast<ReturnToResidenceEvent*> (e) != NULL) {
            return true;
        }
    }
    return false;
...
bool Person::is_in_external_population() {
    return (dynamic_cast<ExternalPopulation*> (population_) != NULL) ? true : false;
}

bool Person::has_birthday_event() {

    BOOST_FOREACH(Event* e, *events()) {
        if (dynamic_cast<BirthdayEvent*> (e) != NULL) {
            return true;
        }
    }
    return false;
...
bool Person::has_update_by_having_drug_event() {
    ...
    BOOST_FOREACH(Event* e, *events()) {
        if (dynamic_cast<UpdateWhenDrugIsPresentEvent*> (e) != NULL) {
            return true;
        }
    }
    return false;
