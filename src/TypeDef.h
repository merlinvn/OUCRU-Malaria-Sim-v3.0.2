...
struct ImmuneSystemInformation {
    ...
    ImmuneSystemInformation() : acquire_rate(-1), decay_rate(-1), duration_for_fully_immune(-1), duration_for_naive(-1),
                                immune_inflation_rate(-1), min_clinical_probability(-1), max_clinical_probability(-1),
                                c_min(-1), c_max(-1),
...
struct ImportationParasiteInfo {
    ...
    ImportationParasiteInfo(int loc, int p_type, int dur, int num) : location(loc), parasite_type_id(p_type),
                                                                     number(num), time(dur) {
    };
...
struct ImportationParasitePeriodicallyInfo {
    ...
    ImportationParasitePeriodicallyInfo(int loc, int p_type, int dur, int num) : location(loc),
