...
Config::Config(Model *model) : model_(model), strategy_(NULL), strategy_db_(), therapy_db_(), drug_db_(NULL), genotype_db_(NULL), initial_parasite_info_(), tme_strategy_(NULL), location_db_()
{
    total_time_ = -1;
...
void Config::read_from_file(const std::string &config_file_name)
{
    ...
    {
        v_distance_by_location_[from_location].resize(number_of_location);
...
void Config::read_location_db_using_normal_info(const YAML::Node &config)
{
    ...
    for (int i = 0; i < number_of_locations_; i++)
    {
        ...
        d->coordinate.x = from_location % grid_x;
        d->coordinate.y = from_location / grid_y_;
...
void Config::read_location_db_using_coordinate_info(const YAML::Node &config)
{
    ...
    for (int i = 0; i < config.size(); i++)
    {
        ...
        for (int i = 0; i < n["districts_info"]["pop_proportion_by_districts"].size(); i++)
        {
            ...
            district_id++;
            d->province_id = p->id;
            d->pop_size = p->pop_size * n["districts_info"]["pop_proportion_by_districts"][i].as<double>;
            d->beta = n["districts_info"]["beta_by_district"][i].as<double>;
            d->coordinate.x = n["districts_info"]["coordinates_by_district"][i][0];
            d->coordinate.y = n["districts_info"]["coordinates_by_district"][i][1];
            location_db_.districts.push_back(d);
        }
    }
}
