...
ImportationEvent::ImportationEvent(const int &location, const int &execute_at, const int &genotype_id,
                                   const int &number_of_cases) : location_(location), execute_at_(execute_at),
...
//    std::cout << number_of_cases_ << std::endl;
    ...
    for (int i = 0; i < number_of_importation_cases; i++) {
        ...
        ClonalParasitePopulation *blood_parasite = p->add_new_parasite_to_blood(
                Model::CONFIG->parasite_db()->get(genotype_id_));
