...
void PersonIndexByLocationMovingLevel::remove_without_set_index(Person* p) {
	vPerson_[p->location()][p->moving_level()].back()->PersonIndexByLocationMovingLevelHandler::set_index(p->PersonIndexByLocationMovingLevelHandler::index());
	vPerson_[p->location()][p->moving_level()][p->PersonIndexByLocationMovingLevelHandler::index()] = vPerson_[p->location()][p->moving_level()].back();
...
void PersonIndexByLocationMovingLevel::update() {
	for (int location = 0; location < Model::CONFIG->number_of_locations(); location++) {
		for (int ml=0; ml < Model::CONFIG->spatial_information().number_of_moving_levels; ml++) {
