...
void ParasiteDatabase::initialize_mating_matrix() { // TODO:single t in mating
    ...
    for (int m = 0; m < size; m++) {
        for (int f = 0; f < size; f++) {
            mating_matrix_[m][f] = generate_offspring_parasite_density(genotype_db_[m]->resistance_bit_string(), genotype_db_[f]->resistance_bit_string());
        }
    }

}

std::vector<double> ParasiteDatabase::generate_offspring_parasite_density(const boost::dynamic_bitset<>& m, const boost::dynamic_bitset<>& f) {
