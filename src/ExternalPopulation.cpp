...
void ExternalPopulation::initialize() {
    if (model() != NULL) {
        ...
        PersonIndexByLocationStateAgeClass *p_index_by_l_s_a = new PersonIndexByLocationStateAgeClass(
                number_of_location, number_of_hoststate, number_of_ageclasses);
    ...
    for (int parasite_type_id = 0; parasite_type_id < Model::CONFIG->number_of_parasite_types(); parasite_type_id++) {
        ...
        for (int i = 0; i < all_persons()->vPerson().size(); i++) {
            ...
            if (p->host_state() != Person::DEAD) {
                int number_of_true_bites = Model::RANDOM->random_poisson(
                        get_current_daily_EIR_by_location(Model::SCHEDULER->current_time(), p->location(),
                                                          parasite_type_id));

                int number_of_infectious_bites = Model::RANDOM->random_binomial(
                        Model::CONFIG->p_infection_from_an_infectious_bite(), number_of_true_bites);
...
void ExternalPopulation::notify_change_in_force_of_infection(const int &location, const int &parasite_type_id,
...
double ExternalPopulation::get_current_daily_EIR_by_location(const int &current_time, const int &location,
                                                             const int &parasite_type_id) {
    //TODO::read from config class
    if (parasite_type_id != 0) {
        return 0;
    }
    double seasonality = Model::CONFIG->seasonality(current_time,
                                                    Model::CONFIG->spatial_external_population_information().seasonal_EIR.a[location],
                                                    Model::CONFIG->spatial_external_population_information().seasonal_EIR.phi[location]);
