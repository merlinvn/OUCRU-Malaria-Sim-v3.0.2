...
void SingleHostClonalParasitePopulations::change_all_infection_force(const double& sign) {
    ...
    for (int p = 0; p < Model::CONFIG->number_of_parasite_types(); p++) {
        person_->notify_change_in_force_of_infection(sign, p, relative_effective_parasite_density_->at(p), log10_total_relative_density_);
    }
...
void SingleHostClonalParasitePopulations::get_parasites_profiles(std::vector<double>& relative_parasite_density, double& log10_total_relative_density) {
    int i = 0;

    while ((i < parasites_->size()) && (parasites_->at(i)->get_log10_relative_density() == ClonalParasitePopulation::LOG_ZERO_PARASITE_DENSITY)) {
...
void SingleHostClonalParasitePopulations::change_all_parasite_update_function(ParasiteDensityUpdateFunction* from, ParasiteDensityUpdateFunction* to) {
    for (int i = 0; i < parasites_->size(); i++) {
...
void SingleHostClonalParasitePopulations::clear_cured_parasites() {
    ...
    for (int i = parasites_->size() - 1; i >= 0; i--) {
        if (parasites_->at(i)->last_update_log10_parasite_density() <= Model::CONFIG->log_parasite_density_level().log_parasite_density_cured + 0.00001) {
            remove(i);
        }
    }
}
...
bool SingleHostClonalParasitePopulations::has_detectable_parasite() {
    for (int i = 0; i < parasites_->size(); i++) {
        if (parasites_->at(i)->last_update_log10_parasite_density() >= Model::CONFIG->log_parasite_density_level().log_parasite_density_detectable) {
            return true;
        }
    }
    return false;
