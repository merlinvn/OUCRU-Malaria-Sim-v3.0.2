...
void MoveParasiteToBloodEvent::schedule_event(Scheduler *scheduler, Person *p, Genotype *infection_type,
                                              const int &time) {
    if (scheduler != NULL) {
...
void MoveParasiteToBloodEvent::execute() {
    ...
    new_parasite->set_last_update_log10_parasite_density(Model::RANDOM->random_normal_truncated(
            Model::CONFIG->log_parasite_density_level().log_parasite_density_asymptomatic, 0.5));
