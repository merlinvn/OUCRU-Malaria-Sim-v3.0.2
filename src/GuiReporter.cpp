...
void GuiReporter::before_run() {
    ...
    CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % prevalanceChart % 0), "Prevalance & Immune",
                   "Time", "Integer", 0, Model::CONFIG->total_time(), "Prevalance", "Double", 0, 0.1, "Immune level",
                   "Double", 0, 0.3);

    //    CreateNewChart("BarChart", boost::str(boost::format("%1%%2%") % ageClassChart % 0), "Age Class", "Age Class", "Category", 0, 0, "Percent", "Double", 0, 1);
    CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % meanMOIChart % 0), "MeanMOI", "Time", "Integer", 0,
                   Model::CONFIG->total_time(), "Mean MOI", "Double", 0, 8);

    CreateNewChart("BarChart", boost::str(boost::format("%1%%2%") % multiInfectionChart % 0), "Multi Infection Chart",
                   "Number of Parasite Population", "Category", 0, 0, "Number of Person", "Integer", 0, 1);


    CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % monoResistanceChart % 0), "Mono Resistance",
                   "Time", "Integer", 0, Model::CONFIG->total_time(), "Biting-rate weighted FOI", "Double", 0, 1);
    CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % doubleResistanceChart % 0), "Double Resistance",
                   "Time", "Integer", 0, Model::CONFIG->total_time(), "Biting-rate weighted FOI", "Double", 0, 1);
    CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % tripleResistanceChart % 0), "Triple Resistance",
                   "Time", "Integer", 0, Model::CONFIG->total_time(), "Biting-rate weighted FOI", "Double", 0, 1);
    ...
    if (Model::CONFIG->strategy()->is_strategy("CyclingStrategy") ||
        Model::CONFIG->strategy()->is_strategy("AdaptiveCyclingStrategy")) {
    ...
    SendTabData(simulationInfoTab, "PopulationSize", "", "",
                NumberToString<int>(Model::CONFIG->population_size_by_location()[0]), "Population Size = <Data>");
...
void GuiReporter::CreateNewChart(const std::string &chartType, const std::string &chartName,
                                 const std::string &chartTitle, const std::string &xAxisTitle,
                                 const std::string &xAxisType, const int &xRangeFrom, const int &xRangeTo,
                                 const std::string &y1AxisTitle, const std::string &y1AxisType,
...
void GuiReporter::CreateWindowsTableTab(const std::string &command, const std::string &tabType,
                                        const std::string &tabName, const std::string &tabTitle,
                                        const std::string &switchRowWithColumn) {
    std::cout << command << SEPARATOR <<
    tabType << SEPARATOR <<
    tabName << SEPARATOR <<
    tabTitle << SEPARATOR <<
...
void GuiReporter::send_prevalence_chart_data_for_location(const int &location) {
    ...
    SendLineChartData(chart_name, "Blood Slide", NumberToString<int>(Model::SCHEDULER->current_time()),
                      NumberToString<double>(Model::DATA_COLLECTOR->blood_slide_prevalence_by_location()[location]),
                      "0");
...
void GuiReporter::send_mono_resistance_chart_data(const int &location) {
    ...
    for (it = Model::CONFIG->parasite_db()->genotype_db().begin();
         it != Model::CONFIG->parasite_db()->genotype_db().end(); it++) {
