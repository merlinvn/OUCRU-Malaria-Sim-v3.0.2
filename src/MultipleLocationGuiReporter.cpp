...
void MultipleLocationGuiReporter::before_run() {
    ...
    for (int location = 0; location < Model::CONFIG->number_of_locations(); location++) {
        ...
        CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % prevalanceChart % location),
                       boost::str(boost::format("Village %1%") % (location + 1)), "Time", "Integer", 0,
                       Model::CONFIG->total_time(), "Prevalance", "Double", 0, 0.1, "EIR per person day", "Double", 0,
                       0.1);
    }

    for (int location = 0; location < Model::CONFIG->number_of_locations(); location++) {
        CreateNewChart("LineChart", boost::str(boost::format("%1%%2%") % monoResistanceChart % location),
                       boost::str(boost::format("Mono Resistance for location %1%") % location), "Time", "Integer", 0,
                       Model::CONFIG->total_time(), "Biting-rate weighted FOI", "Double", 0, 1);
    }
