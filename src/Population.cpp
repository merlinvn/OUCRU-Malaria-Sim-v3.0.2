...
void Population::perform_infection_event() {
    ...
    for (int loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
        for (int parasite_type_id = 0;
            ...
            double newBeta = Model::CONFIG->beta()[loc] * Model::CONFIG->seasonality(Model::SCHEDULER->current_time(),
                                                                                     Model::CONFIG->seasonal_beta().a[loc],
                                                                                     Model::CONFIG->seasonal_beta().phi[loc]);
            ...
            for (int age_class = 0; age_class < Model::CONFIG->initial_age_structure().size(); age_class++) {
                ...
                if (age_class == Model::CONFIG->initial_age_structure().size() - 1) {
                    ...
                    number_of_individual_by_loc_ageclass =
                            popsize_by_location * Model::CONFIG->age_distribution_by_location()[loc][age_class];
...
void Population::perform_circulation_for_1_location(const int &from_location, const int &target_location,
    ...
    for (int i = 0; i < Model::CONFIG->spatial_information().number_of_moving_levels; i++) {
        double temp =
                Model::CONFIG->spatial_information().v_moving_level_value[i] * pi->vPerson()[from_location][i].size();
        vLevelDensity.push_back(temp);
    }
...
void Population::initialize_person_indices() {
    ...
    PersonIndexByLocationMovingLevel *p_index_location_moving_level = new PersonIndexByLocationMovingLevel(
            number_of_location, Model::CONFIG->spatial_information().number_of_moving_levels);
    ...
    PersonIndexByLocationExternalPopulationMovingLevel *p_index_location_external_moving_level = new PersonIndexByLocationExternalPopulationMovingLevel(
            number_of_location, Model::CONFIG->spatial_external_population_information().number_of_moving_levels);
...
void Population::perform_moving_to_external_population_event() {
    ...
    for (int location = 0; location < Model::CONFIG->number_of_locations(); location++) {
        ...
        double poisson_means =
                size(location) * Model::CONFIG->spatial_external_population_information().circulation_percent[location];
        if (poisson_means == 0)continue;
        ...
        for (int i = 0; i < Model::CONFIG->spatial_external_population_information().number_of_moving_levels; i++) {
            double temp = Model::CONFIG->spatial_external_population_information().v_moving_level_value[i] *
                          pi->vPerson()[location][i].size();
            vLevelDensity.push_back(temp);
        }
