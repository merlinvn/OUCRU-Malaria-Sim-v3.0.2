...
#define CLEAR(TYPE, P) case TYPE: if (P) delete [] P; P = 0; break;
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const char *list, int _size) : insensitive(0), op(_op),
                                                                                              size(_size), s1(0),
                                                                                              type(_type), quiet(0) {
        id = ezOptionParserIDGenerator::instance().next();
        s1 = new char[size];
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const unsigned char *list, int _size) : insensitive(0),
                                                                                                       op(_op),
                                                                                                       size(_size),
                                                                                                       u1(0),
                                                                                                       type(_type),
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const unsigned short *list, int _size) : insensitive(0),
                                                                                                        op(_op),
                                                                                                        size(_size),
                                                                                                        u2(0),
                                                                                                        type(_type),
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const unsigned int *list, int _size) : insensitive(0),
                                                                                                      op(_op),
                                                                                                      size(_size),
                                                                                                      u4(0),
                                                                                                      type(_type),
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const long long *list, int _size) : insensitive(0),
                                                                                                   op(_op), size(_size),
                                                                                                   s8(0), type(_type),
                                                                                                   quiet(0) {
        id = ezOptionParserIDGenerator::instance().next();
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const unsigned long long *list, int _size) : insensitive(
            0), op(_op), size(_size), u8(0), type(_type), quiet(0) {
        id = ezOptionParserIDGenerator::instance().next();
    ...
    ezOptionValidator::ezOptionValidator(char _type, char _op, const char **list, int _size, bool _insensitive)
            : insensitive(_insensitive), op(_op), size(_size), t(0), type(_type), quiet(0) {
        id = ezOptionParserIDGenerator::instance().next();
    ...
    ezOptionValidator::ezOptionValidator(const char *_type, const char *_op, const char *_list, bool _insensitive)
            : insensitive(_insensitive), size(0), t(0), type(0), quiet(0) {
        id = ezOptionParserIDGenerator::instance().next();
...
#define ToArray(T, P, Y) case T: P = new Y[size]; To##T(strings, P, size); FreeStrings(); break;
        switch (type) {
            ToArray(S1, s1, char);
            ToArray(U1, u1, unsigned
                    char);
            ToArray(S2, s2, short);
            ToArray(U2, u2, unsigned
                    short);
            ToArray(S4, s4, int);
            ToArray(U4, u4, unsigned
                    int);
            ToArray(S8, s8, long
                    long);
            ToArray(U8, u8, unsigned
                    long
                    long);
            ToArray(F, f, float);
            ToArray(D, d, double);
        ...
        if (type != T) {
            switch (type) {
                case S1: CHECKRANGE(S1, char);
                    break;
                case U1: CHECKRANGE(U1, unsigned
                        char);
                    break;
                case S2: CHECKRANGE(S2, short);
                    break;
                case U2: CHECKRANGE(U2, unsigned
                        short);
                    break;
                case S4: CHECKRANGE(S4, int);
                    break;
                case U4: CHECKRANGE(U4, unsigned
                        int);
                    break;
                case S8: {
                ...
                case F: {
                    ...
                    if (dvalue > dmax) {
                        if (!quiet)
                            std::cerr << "ERROR: Invalid value " << dvalue << " is greater than datatype max " <<
                            dmax << ".\n";
                        return false;
                    }
                }
                ...
                case D: {
                    ...
                    if (ldvalue > ldmax) {
                        if (!quiet)
                            std::cerr << "ERROR: Invalid value " << ldvalue << " is greater than datatype max " <<
                            ldmax << ".\n";
                        return false;
                    }
                }
  ...
  switch (op) {\
    ...
    case GTLE:\
      ...
      case NOOP: case IN: default: break;\
        ...
        switch (type) {
            case U1: VALIDATE(unsigned
                                      char, int, u1);
                break;
            case S1: VALIDATE(char, int, s1);
                break;
            case U2: VALIDATE(unsigned
                                      short, int, u2);
                break;
            case S2: VALIDATE(short, int, s2);
                break;
            case U4: VALIDATE(unsigned
                                      int, unsigned
                                      int, u4);
                break;
            case S4: VALIDATE(int, int, s4);
                break;
            case U8: VALIDATE(unsigned
                                      long
                                      long, unsigned
                                      long
                                      long, u8);
                break;
            case S8: VALIDATE(long
                                      long, long
                                      long, s8);
                break;
